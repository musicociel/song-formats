...
const registerToken = (tokenToPitch, pitchToToken, tokens, pitch) => {
  const tokenInfo = {
    tokens,
    pitch
  };
  tokens.forEach(opt => tokenToPitch[opt.toLowerCase()] = tokenInfo);
  pitchToToken[pitch] = tokenInfo;
};

const registerNote = (tokensString, pitch) => registerToken(noteTokenToPitch, notePitchToToken, tokensString, pitch);
const registerAlteration = (tokensString, pitch: Alteration) => registerToken(alterationTokenToPitch, alterationPitchToToken, tokensString, pitch);
...
export const noteAlteration = Object.keys(alterationTokenToPitch).filter(alt => !!alt).join('|');
...
export const parseAlteration = (alteration): Alteration | undefined => {
...
export const checkAndTransposeNote = (inputNote: Note, options: NoteCheckAndTransposeOptions = {}): Note => {
  let pitch = inputNote[0] + (options.transpose || 0);
  if (options.mod12) {
    pitch = mod12(pitch);
  }
  let alterationPitch = inputNote[1];
  if (options.resetAlterations || !alterationPitch || !alterationPitchToToken[alterationPitch]) {
    alterationPitch = 0;
  }
...
export const formatNote = (inputNote: Note, options: NoteFormatOptions = {}) => {
  ...
  return `${noteInfo.tokens[options.doReMi ? 1 : 0]}${alterationInfo.tokens[0]}`;
};
