import {Chord, checkAndTransposeChord, ChordCheckAndTransposeOptions} from './chord';
import {Note, checkAndTransposeNote, NoteCheckAndTransposeOptions} from './note';
...
export type VoiceConfig = LyricsConfig | ChordsConfig | NotesConfig | CommentsConfig | TimingConfig;

export type VoiceDefinition = VoiceType | {
  type: VoiceType;
  title?: string;
  config?: VoiceConfig;
...
export interface Event {
  [voiceName: string]: LyricsEvent | ChordsEvent | NotesEvent | CommentsEvent | TimingEvent;
}
...
export interface SheetMusicCheckAndTransposeOptions extends ChordCheckAndTransposeOptions, NoteCheckAndTransposeOptions {};

export const getVoiceType = (sheetMusic: SheetMusic, voiceName: string): VoiceType | undefined => {
...
export const checkAndTransposeSheetMusic = (sheetMusic: SheetMusic, options: SheetMusicCheckAndTransposeOptions = {}): SheetMusic => {
  ...
  sheetMusicResult.parts = sheetMusicResult.parts.map(part => {
    ...
    partResult.content = partResult.content.map(events => events.map(event => {
      const eventResult = {...event};
      for (const chordVoice of chordVoices) {
        const chordEvent = eventResult[chordVoice] as ChordsEvent;
        if (chordEvent) {
          eventResult[chordVoice] = checkAndTransposeChord(chordEvent, options);
        }
      }
      for (const noteVoice of noteVoices) {
        const noteEvent = eventResult[noteVoice] as NotesEvent;
        if (noteEvent) {
          eventResult[noteVoice] = noteEvent.map(note => checkAndTransposeNote(note, options));
        }
      }
      return eventResult;
    }));
