import {note, parseNote, formatNote, checkAndTransposeNote, ParsedNote, Note, NoteFormatOptions, NoteCheckAndTransposeOptions} from './note';
...
const transposeChordKey = (key: number, transposition: number) => {
  ...
  return chordKeyMask & ((key << transposition) | (key >>> (12 - transposition)));
};
...
const removeUnicodeAlterations = (text) => text.replace(/\u266D/g, 'b').replace(/\u266F/g, '#');
...
const chordRegExp = new RegExp(`^(${note})([-+#\(\)a-zA-Z0-9\u266D\u266F°]*(?:/[0-9]+)?)(/(${note}))?$`, 'i');
...
export interface ChordCheckAndTransposeOptions extends NoteCheckAndTransposeOptions {
  skipNormalizingChordTypes?: boolean;
  acceptUnknownChords?: boolean;
}

export const checkAndTransposeChord = (chord: Chord, options: ChordCheckAndTransposeOptions = {}): Chord => {
...
export interface ChordFormatOptions extends ChordCheckAndTransposeOptions, NoteFormatOptions {}

export const formatChord = (chord: Chord, options?: ChordFormatOptions): string => {
  ...
  return `${formatNote(root, remainingOptions)}${type}${ bass ? `/${formatNote(bass, remainingOptions)}` : ''}`;
};

const buildDetectedChords = (possibleChords: ChordDetectorResult[], bassPitch): Chord[] => {
  ...
  possibleChords.forEach((chord) => {
    if (chord.root === bassPitch) {
      ...
      bassNotMatchingRoot.push([[chord.root, 0], chord.typeDetails.names[0], [bassPitch, 0]]);
    }
  });
