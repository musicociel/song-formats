import {Song, Part, SongAuthor, Event, PartType} from '../song';
import {parseChord, Chord} from '../chord';
...
export class PartBuilder {
  ...
  constructor(private _songBuilder: SongBuilder, name) {
    this._part = {
      name,
      content: []
    };
  ...
  _addTextEvent(voiceType, text, voice) {
  ...
  addLyrics(lyrics, voice = 'lyrics') {
    return this._addTextEvent('lyrics', lyrics, voice);
  }

  addComments(comments, voice = 'comments') {
...
export class SongBuilder {
  ...
  addAuthor(name, type?: 'composer' | 'lyricist' | 'arranger') {
    name = name ? name.trim() : '';
    if (name) {
  ...
  splitAndAddAuthors(authors) {
    if (authors) {
      authors.split(/[-\u2013&,;\/]/).forEach(author => this.addAuthor(author));
    }
  }
  ...
  checkVoice(name, type) {
    ...
    const voiceType = typeof voiceDefinition === 'string' ? voiceDefinition : voiceDefinition.type;
    if (voiceType !== type) {
      throw new Error(`Voice ${name} already exists with type ${voiceType}, so it cannot contain events of type ${type}.`);
    }
  }
